...
namespace DashBoard.Models.Operation
{
    public class Invoice : OperationBase, IInvoice
    {
        ...
        {
            var context=new ContextManager();
            var query = (from invoice in context.Invoice
        ...
        public void Edit(InvoiceViewModel invoice)
        {
            ContextManager contextManager = new ContextManager();
            var query = contextManager.Invoice.Single(X => X.Id == invoice.Id);
            query.Id = invoice.Id;
            query.SentToAccount= invoice.SentToAccount;
            query.Company = invoice.Company;
            query.YearFiled = invoice.YearFiled;
            query.ClaimJul = invoice.ClaimJul;
            query.InvoicedJul = invoice.InvoicedJul;
            query.ClaimAug = invoice.ClaimAug;
            query.InvoicedAug = invoice.InvoicedAug;
            query.ClaimSep = invoice.ClaimSep;
            query.InvoicedSep = invoice.InvoicedSep;
            query.ClaimOct = invoice.ClaimOct;
            query.InvoicedOct = invoice.InvoicedOct;
            query.ClaimNov = invoice.ClaimNov;
            query.InvoicedNov = invoice.InvoicedNov;
            query.ClaimDec = invoice.ClaimDec;
            query.InvoicedDec = invoice.InvoicedDec;
            query.ClaimJan = invoice.ClaimJan;
            query.InvoicedJan = invoice.InvoicedJan;
            query.ClaimFeb = invoice.ClaimFeb;
            query.InvoicedFeb = invoice.InvoicedFeb;
            query.ClaimMar = invoice.ClaimMar;
            query.InvoicedMar = invoice.InvoicedMar;
            query.ClaimApr = invoice.ClaimApr;
            query.InvoicedApr = invoice.InvoicedApr;
            query.ClaimMay = invoice.ClaimMay;
            query.InvoicedMay = invoice.InvoicedMay;
            query.ClaimJun = invoice.ClaimJun;
            query.InvoicedJun = invoice.ClaimJun;
            contextManager.SaveChanges();
        ...
        {
            ContextManager contextManager = new ContextManager();
            var query = contextManager.Invoice.Single(x => x.Id == id);
            contextManager.Invoice.Remove(query);
            contextManager.SaveChanges();
        }


        public void DownloadExcel(int startcolumn, int endcolumn, int startrow, int endrow, string sheet, List<InvoiceViewModel> lista)
        {
            base.DownloadExcel(startcolumn,endcolumn,startrow,endrow,sheet,lista);
        }
    }
}