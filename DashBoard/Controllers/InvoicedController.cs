...
namespace DashBoard.Controllers
{
    public class InvoicedController : BaseController
    {
        ...
        [HttpGet]
        public ActionResult ViewGrid(ViewModel.InvoiceViewModel vm)
        {
            List<InvoiceViewModel> get = _invoice.GetAll().Where(x => (vm.SentToAccount == null || x.SentToAccount.ToLower().Contains(vm.SentToAccount.ToLower()))).ToList();
            var vmodel = new InvoiceVm();
            vmodel.InvoiceViewModel = get;
            return View("_Grid", vmodel);
        }
        
        [HttpGet]
        public ActionResult Create()
        {
            var vm = new ViewModel.InvoiceViewModel();
            return View("Create", vm);
        }

        [HttpPost]
        public ActionResult Create(ViewModel.InvoiceViewModel invoice)
        {
            _invoice.AddInvoiced(invoice);
        ...
        {
            var result = _invoice.GetAll().Single(x => x.Id == id);
            return View(result);
        }

        [HttpPost]
        public ActionResult Edit(ViewModel.InvoiceViewModel invoice)
        {
            _invoice.Edit(invoice);
